...
export const metadata = createMetadata({
  title: "South Asia CPIN & Country Guidance 2025 | Asylum Solicitor Guide",
  description:
  ...
  return (
    <>
      ...
      >
        ...
        <h2 className="mt-10 text-xl font-bold text-[#1A2138]">Current South Asia CPINs</h2>
        <div className="mt-4">
          <ResponsiveTableWrap>
            <table className="w-full min-w-[500px] border-collapse text-sm">
              <caption className="mb-3 text-left text-base font-semibold text-[#1A2138]">
              ...
              <thead>
                <tr className="border-b border-[#D4CDC2] bg-[#E8E2D8]">
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-[#1A2138]">CPIN Topics Available</th>
                </tr>
              </thead>
              <tbody>
                {cpinRows.map((row) => (
                  <tr key={row.country} className="border-b border-[#D4CDC2]">
                    <td className="px-4 py-3 font-medium text-[#3A4250]">{row.country}</td>
                    <td className="px-4 py-3 text-[#3A4250]">{row.topics}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <p className="mt-4 text-sm text-[#3A4250] italic">{cpinNote}</p>

        <h2 className="mt-10 text-xl font-bold text-[#1A2138]">Country Guidance Cases</h2>
        <ul className="mt-4 space-y-4">
          {countryGuidanceCases.map((cg) => (
            <li key={cg.citation} className="rounded-none border border-[#D4CDC2] bg-[#E8E2D8] p-4">
              <p className="font-semibold text-[#1A2138]">{cg.citation}</p>
              <p className="mt-1 text-sm text-[#3A4250]">{cg.summary}</p>
            </li>
          ))}
        </ul>
        <p className="mt-4 text-[#3A4250]">
        ...
        <h2 className="mt-10 text-xl font-bold text-[#1A2138]">Post-August 2024 Bangladesh CPIN Gap</h2>
        ...
        <h2 className="mt-10 text-xl font-bold text-[#1A2138]">How Expert Reports Relate to CPINs</h2>
        <p className="mt-4 text-[#3A4250] leading-relaxed">
