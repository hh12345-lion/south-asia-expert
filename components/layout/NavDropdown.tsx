...
export function NavDropdown({ label, href, items }: NavDropdownProps) {
  return (
    <div className="group relative self-stretch">
      <Link
        href={href}
        className="inline-flex h-full min-h-[48px] items-center gap-1 px-3 text-sm text-[#3A4250] transition hover:text-[#1A2138]"
      >
        <span>{label}</span>
      ...
      <div className="pointer-events-none invisible absolute left-0 top-full z-50 min-w-[16rem] pt-1 opacity-0 transition-[opacity,visibility] duration-150 group-hover:pointer-events-auto group-hover:visible group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:visible group-focus-within:opacity-100">
        <ul className="menu-list max-h-[min(70vh,22rem)] overflow-y-auto border border-[#D4CDC2] bg-white">
          {items.map((item) => (
            <li key={item.href} className="border-b border-[#D4CDC2] last:border-b-0">
              <Link
                href={item.href}
                className="flex min-h-[44px] items-center px-4 py-2 text-sm text-[#3A4250] hover:bg-[#E8E2D8] hover:text-[#1A2138]"
              >
                {item.label}
              </Link>
            </li>
