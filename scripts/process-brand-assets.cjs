...
async function main() {
  ...
  for (const [size, out] of [
    ...
    await sharp({
      create: {
        ...
        height: size,
        channels: 4,
        background: { r: 26, g: 33, b: 56, alpha: 1 }, // #1A2138
      },
    })
  ...
  // Replace default Next favicon.ico with brand icon
  ...
  await sharp(path.join(icons, "icon-32.png")).png().toFile(path.join("public", "favicon.ico"));
  ...
  // Also overwrite app/favicon.ico if present (Next prefers it)
  try {
    await sharp(path.join(icons, "icon-32.png")).png().toFile(path.join("app", "favicon.ico"));
  } catch {}
